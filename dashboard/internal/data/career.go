...
var (
	...
	reTlDrPlain      = regexp.MustCompile(`(?m)^TL;DR:\s*(.+)`)
	reRemote         = regexp.MustCompile(`(?i)\*\*Remote\*\*\s*\|\s*(.+)`)
	reComp           = regexp.MustCompile(`(?i)\*\*Comp\*\*\s*\|\s*(.+)`)
	reCompSalary     = regexp.MustCompile(`(?i)\|\s*Salary\s*\|\s*(.+?)\s*\|`)
...
func ParseApplications(careerOpsPath string) []model.CareerApplication {
	...
	// Enrich with report summary data (Archetype, TLDR, Remote, CompEstimate)
	for i := range apps {
		if apps[i].ReportPath == "" {
			continue
		}
		archetype, tldr, remote, comp := LoadReportSummary(careerOpsPath, apps[i].ReportPath)
		apps[i].Archetype = archetype
		apps[i].TlDr = tldr
		apps[i].Remote = remote
		apps[i].CompEstimate = comp
		// Fallback: use Notes from tracker as TLDR if report has no dedicated TLDR field
		if apps[i].TlDr == "" && apps[i].Notes != "" {
			apps[i].TlDr = apps[i].Notes
		}
	}
...
func LoadReportSummary(careerOpsPath, reportPath string) (archetype, tldr, remote, comp string) {
	...
	// Try table-format TL;DR first (most reports), then colon format, then plain
	if m := reTlDr.FindStringSubmatch(text); m != nil {
		tldr = cleanTableCell(m[1])
	} else if m := reTlDrColon.FindStringSubmatch(text); m != nil {
		tldr = cleanTableCell(m[1])
	} else if m := reTlDrPlain.FindStringSubmatch(text); m != nil {
		tldr = cleanTableCell(m[1])
	}
	...
	} else if m := reCompSalary.FindStringSubmatch(text); m != nil {
		comp = cleanTableCell(m[1])
	}
