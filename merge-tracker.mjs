...
function validateStatus(status) {
  ...
  if (/^(dup(licado)?|repost)/i.test(lower)) return 'Discarded';
...
function extractReportNum(reportStr) {
  // Handle [#001](reports/001-unite-us.md) format
  let m = reportStr.match(/\[#?(\d+)\]/);
  if (m) return parseInt(m[1]);
  // Handle plain "001-unite-us" format
  m = reportStr.match(/^(\d+)/);
  return m ? parseInt(m[1]) : null;
}

function fmtReportLink(slug) {
  // "001-unite-us" → "[#001](reports/001-unite-us.md)"
  const m = slug.match(/^(\d+)/);
  const num = m ? m[1] : slug;
  return `[#${num}](reports/${slug}.md)`;
}
...
function parseTsvContent(content, filename) {
  ...
  if (content.startsWith('|')) {
    ...
    const col4LooksLikeStatus = /^(evaluated|applied|responded|interview|offer|rejected|discarded|skip|cerrada|dup|repost|hold|monitor)/i.test(col4);
    const col5LooksLikeStatus = /^(evaluated|applied|responded|interview|offer|rejected|discarded|skip|cerrada|dup|repost|hold|monitor)/i.test(col5);
...
for (const file of tsvFiles) {
  ...
  if (duplicate) {
    ...
    if (newScore > oldScore) {
      ...
      if (lineIdx >= 0) {
        const updatedLine = `| ${duplicate.num} | ${addition.date} | ${addition.company} | ${addition.role} | ${addition.score} | ${duplicate.status} | ${duplicate.pdf} | ${fmtReportLink(addition.report)} | Re-eval ${addition.date} (${oldScore}→${newScore}). ${addition.notes} |`;
    ...
    const newLine = `| ${entryNum} | ${addition.date} | ${addition.company} | ${addition.role} | ${addition.score} | ${addition.status} | ${addition.pdf} | ${fmtReportLink(addition.report)} | ${addition.notes} |`;
