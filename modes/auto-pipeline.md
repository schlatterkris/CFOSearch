...
When the user pastes a JD (text or URL) without explicit sub-command, execute the ENTIRE pipeline in sequence:
...
**If no method works:** Ask the candidate to manually paste the JD or share a screenshot.

**If the input is JD text** (not URL): use directly, no fetch needed.

## Step 1 — A-F Evaluation
Execute exactly like the `offer` mode (read `modes/evaluate.md` for all blocks A-F).

## Step 2 — Save Report .md
Save the complete evaluation to `reports/{###}-{company-slug}-{YYYY-MM-DD}.md` (see format in `modes/evaluate.md`).
...
1. **Extract form questions**: Use Playwright to navigate to the form and take snapshot. If they can't be extracted, use generic questions.
...
**Language**: Always in the JD language (EN default). Apply `/tech-translate`.
