...
AI-powered job search automation built on Google Gemini: pipeline tracking, offer evaluation, CV generation, portal scanning, batch processing.
...
**Exception for batch workers (Gemini API):** Playwright is not available in batch mode. Use WebFetch as fallback and mark the report header with `**Verification:** unconfirmed (batch mode)`. The user can verify manually later.
